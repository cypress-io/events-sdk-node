    ...
    addPersistentQueueProcessor() {
        ...
        this.pQueue.process((job, done) => {
            ...
            if (jobData.attempts >= maxAttempts) {
                ...
                }, Math.min(30000, 2 ** jobData.attempts * 1000), this.axiosInstance, this.host, this.path, this.gzip);
            }
        });
    }
    /**
    ...
    async flush(callback) {
        ...
        if (this.pQueue && this.pQueueInitialized) {
            ...
            this.pQueue
                ...
                this.state = 'idle';
                this.logger.error(`failed to push to redis queue, in-memory queue size: ${this.queue.length}`);
                throw error;
            });
        }
        else if (!this.pQueue) {
            this.pendingFlush = this.axiosInstance
                ...
                const isDuringTestExecution = err &&
                    err.response &&
